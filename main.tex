...
We use rule (R-IApp) for invocations $e_1 \ e_2$ of overloaded identifiers. Thus $e_1$ evaluates to a list of typed expressions $S$. If there exists an instance $(e^\prime, \ \sigma_T) \in S$ which's type $T$ matches the type of the argument $e_2$, we take $e^\prime$ and apply $v_2$ (evaluated $e_2$) to it. To determine if (R-App) or (R-IApp) should be used for some application $e_1 \ e_2$, we can simply check if $e_1$ evaluates to a list $S$ or a single value $v$. 
